...
class RutaEscanearFacialView(APIView):
    permission_classes = [IsAuthenticated]
    ...
    def post(self, request):
        ...
        rut = request.data.get('rut', '').strip()
        accion = request.data.get('accion', 'abordar')
        score = request.data.get('score', 0.95)
        ...
        if not estudiante:
            return Response({'message': 'Estudiante no encontrado en el sistema.'}, status=status.HTTP_404_NOT_FOUND)

        porcentaje = int(float(score) * 100) if score is not None else 95

        if accion == 'abordar':
            titulo = "🎒 Estudiante Abordó (FaceScanner)"
            mensaje = f"¡{estudiante.nombre} {estudiante.apellido} ha abordado el furgón escolar! (Verificado con Reconocimiento Facial: {porcentaje}% coincidencia)"
            tipo = "estudiante_abordo"
        else:
            titulo = "🏠 Estudiante Llegó (FaceScanner)"
            mensaje = f"¡{estudiante.nombre} {estudiante.apellido} ha llegado a su destino! (Verificado con Reconocimiento Facial: {porcentaje}% coincidencia)"
            tipo = "estudiante_llego"
        ...
        return Response({
            'message': f"Escaneo facial procesado exitosamente: {accion}",
